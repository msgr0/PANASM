...
def main(args):
  ...
  out_prediction = pd.DataFrame(colums = header)
    ...
    for contig in contigs:
      ...
      try:
        ...
        try:
          ...
          print("Halted cause of length error:", e1, e2, "check your gfa file.")
          sys.exit(1)
        out_prediction.loc[len(out_prediction)] = [plasmid, contig, contig_len]
  out_prediction.to_csv(args.output, index = False, sep = '\t')
    
if __name__ == "__main__":
  parser = ap.ArgumentParser(description="Convert PLASBIn-FLOW prediction to PlasEval.")
